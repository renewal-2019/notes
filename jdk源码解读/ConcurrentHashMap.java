...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {//通过哈希计算出一个表中的位置
    ...
    private final Node<K,V>[] initTable() {
        Node<K,V>[] tab; int sc;
        while ((tab = table) == null || tab.length == 0) {//table未初始化时,则进入while
            if ((sc = sizeCtl) < 0)//sizeCtl初始值为0，当小于0的时候表示在别的线程在初始化表或扩展表
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {//CAS操作; SIZECTL:当前对象内存偏移量; sc:期望值; -1表示要替换的值,-1表示要初始化表
                try {
    ...
    private final void treeifyBin(Node<K,V>[] tab, int index) {
        Node<K,V> b; int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY) //数组长度 < 64
                tryPresize(n << 1); //扩容
            else if ((b = tabAt(tab, index)) != null && b.hash >= 0) {
                synchronized (b) { //同步, 将节点处链表转换为树
                    if (tabAt(tab, index) == b) {
                        TreeNode<K,V> hd = null, tl = null; // hd: head
